    ...
    <>
      <div className="flex flex-col-reverse md:flex">
        ...
        <div className="container">
          <form className="bg-white flex fixed left-0 right-0 top-0 z-[8] items-center max-w-[75rem] mr-[10px] pb-[20px] pt-[4px] md:pt-[5px] mx-auto">
          ...
          <div className="md:absolute md:pl-[320px] pt-20 md:left-0 md:right-0">
            <div className="flex flex-wrap gap-[10px]">
              {filteredCards.map((component, index) => (
                <div key={index} className="border p-4 rounded-lg shadow-md bg-white w-full md:w-1/3">
                  <h2 className="text-lg font-bold">{component.title}</h2>
                  <p className="text-gray-700">{component.description}</p>
                  <a href={component.link_url} className="text-blue-500 hover:underline">{component.link_text}</a>
                </div>
              ))}
            </div>
          </div>
